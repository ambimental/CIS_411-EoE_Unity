...
public class ActionsCP2 : MonoBehaviour
{
...
//        {
//            Type type = typeof(ActsCP2);
//            ActsCP2 ClassObject = new ActsCP2();
...
//        GameManager.Instance.cp2ProtectedFromInvasiveAnimal = true;
//    }

//    //Human - Biologist
//    public void a002() //Special Action: Locate and play one animal from the deck or discard pile. Discard after use.
//    {
//        GameManager.Instance.pickCardHolder = GameObject.Find("Computer One Board/Animal Card Placement").transform; //sets parent
...
//        {
//            if (deck.DeckId == GameManager.Instance.computerTwoDeck) //checks to make sure that the players deck is the one that is being used
//            {
...
//                for (int j = 0; j < GameManager.Instance.DiscardPlacementCP2.Count; j++) //this will bes used to check the discard pile and see if there are any animals in it
//                {
//                    if (GameManager.Instance.DiscardPlacementCP2[j].CardType == "Animal")
//                    {
//                        GameManager.Instance.DiscardSelectionList.Add(GameManager.Instance.DiscardPlacementCP2[j]); //if an animal, adds to the discard selection list
//                    }
//                }
...
//                    GameManager.Instance.AnimalPlacementCP2.Add(cardHolder); //adds the selected card to the animal placement list
...
//                    for (int i = 0; i < GameManager.Instance.DiscardPlacementCP2.Count; i++) //goes through the discard pile, finds the card, then removes it from the deck list
//                    {
//                        if (GameManager.Instance.DiscardPlacementCP2[i].CardName == cardHolder.CardName)
//                        {
//                            GameManager.Instance.DiscardPlacementCP2.Remove(cardHolder); //removes the card
//                            break; //just breaks the for loop
//                        }
//                    }
//                }
...
//        GameManager.Instance.cp2ProtectedFromInvasivePlant = true;
//    }

//    //Human - Botanist
//    public void a004() //Special Action: Locate and play one Plant from the deck or discard pile. Discard after use.
//    {
//        GameManager.Instance.pickCardHolder = GameObject.Find("Computer One Board/Plant Card Placement").transform; //sets parent
//        bool deckCard = false; //these two bools will determine where the card came from
//        bool discardCard = false;
...
//                for (int j = 0; j < GameManager.Instance.DiscardPlacementCP2.Count; j++) //this will bes used to check the discard pile and see if there are any animals in it
//                {
//                    if (GameManager.Instance.DiscardPlacementCP2[j].CardType == "Plant")
//                    {
//                        GameManager.Instance.DiscardSelectionList.Add(GameManager.Instance.DiscardPlacementCP2[j]); //if an animal, adds to the discard selection list
//                    }
//                }
...
//                    GameManager.Instance.PlantPlacementCP2.Add(cardHolder); //adds the selected card to the animal placement list
...
//        GameManager.Instance.cp2NoConditionRequirements = true;
//    }

//    //Human - Explorer
//    public void a006() //Special Action: Locate and play one Condition card from the deck or discard pile. Discard after use.
//    {
//        GameManager.Instance.pickCardHolder = GameObject.Find("Computer One Board/Condition Card Placement").transform; //sets parent
//        bool deckCard = false; //these two bools will determine where the card came from
//        bool discardCard = false;
...
//                for (int j = 0; j < GameManager.Instance.DiscardPlacementCP2.Count; j++) //this will bes used to check the discard pile and see if there are any animals in it
//                {
//                    if (GameManager.Instance.DiscardPlacementCP2[j].CardType == "Condition")
//                    {
//                        GameManager.Instance.DiscardSelectionList.Add(GameManager.Instance.DiscardPlacementCP2[j]); //if an animal, adds to the discard selection list
//                    }
//                }
...
//                    GameManager.Instance.ConditionPlacementCP2.Add(cardHolder); //adds the selected card to the animal placement list
...
//    {
//        GameManager.Instance.cp2ProtectedFromBlight = true;
//    }
...
//    {
//        //GameManager.Instance.cp2ProtectedFromExtinction = true; //player will be protected from exitinction while this is true
//    }
...
//    {
//        GameManager.Instance.cp2PausedOneTurn = true; //this will make it so that the players actions will be paused for one round
//    }
...
//    {
//        GameManager.Instance.cp2ProtectedFromWeb = true; //protects the player from the web of life and other standing actions cards
//    }
...
//        GameManager.Instance.cp2PausedOneTurn = true; //will make it so it pauses
//    }

//    //Human - Grandma Moore
//    public void a016() //Standing Action: When Grandma Moore is in play, you no longer need to discard at the end of your turn
//    {
//        GameManager.Instance.cp2NoDiscard = true; //player no longer has to discard
//    }
...
//    {
//        GameManager.Instance.cp2DrawExtraCard = true; //player can now draw an extra card at the beginning of the round
//    }
...
//    {
//        GameManager.Instance.cp2TwentyPointNoDiscard = true; //player can keep one twenty point card in their hand 
//    }
...
//    {
//        GameManager.Instance.cp2ProtectedFromInvertebrate = true; //player is protected from invaive invertebrate
//    }

//    //Human - Entomologist
//    public void a0027() //Special Action: Locate and play one invertebrate from the deck or discard pile. Discard after use.
//    {
//        GameManager.Instance.pickCardHolder = GameObject.Find("Computer One Board/Invertebrate Card Placement").transform; //sets parent
//        bool deckCard = false; //these two bools will determine where the card came from
//        bool discardCard = false;
...
//                for (int j = 0; j < GameManager.Instance.DiscardPlacementCP2.Count; j++) //this will bes used to check the discard pile and see if there are any animals in it
//                {
//                    if (GameManager.Instance.DiscardPlacementCP2[j].CardType == "Invertebrate")
//                    {
//                        GameManager.Instance.DiscardSelectionList.Add(GameManager.Instance.DiscardPlacementCP2[j]); //if an animal, adds to the discard selection list
//                    }
//                }
...
//                    GameManager.Instance.InvertebratePlacementCP2.Add(cardHolder); //adds the selected card to the animal placement list
...
//                for (int j = 0; j < GameManager.Instance.DiscardPlacementCP2.Count; j++) //this will bes used to check the discard pile and see if there are any animals in it
//                {
//                    if (GameManager.Instance.DiscardPlacementCP2[j].PlantType == "Canopy" || GameManager.Instance.DiscardPlacementCP2[j].PlantType == "Understory")
//                    {
...
//    {
//        GameManager.Instance.pickCardHolder = GameObject.Find("Computer Two Board/Animal Card Placement").transform; //sets parent
//        bool deckCard = false; //these two bools will determine where the card came from
//        bool discardCard = false;
...
//                for (int j = 0; j < GameManager.Instance.DiscardPlacementCP2.Count; j++) //this will bes used to check the discard pile and see if there are any animals in it
//                {
//                    if (GameManager.Instance.DiscardPlacementCP2[j].AnimalDiet == "Herbivore")
//                    {
//                        GameManager.Instance.DiscardSelectionList.Add(GameManager.Instance.DiscardPlacement[j]); //if an animal, adds to the discard selection list
//                    }
//                }
...
//    {
//        for (int i = 0; i < GameManager.Instance.MicrobePlacementCP2.Count; i++) //goes through the microbe placement list
//        {
//            if (GameManager.Instance.MicrobePlacementCP2[i].CardName == "Microbe-Spirulina-Algae") //if the correct card is here, sets the plant type to groundcover
//                GameManager.Instance.MicrobePlacementCP2[i].PlantType = "Groundcover";
//        }
//    }
